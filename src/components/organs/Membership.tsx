...
const Membership = () => {
  ...
  return (
    <section className="w-full h-auto py-20 md:py-28 lg:py-40 bg-zinc-950 flex flex-col md:gap-28 gap-20 justify-center items-center">
      ...
      <main className="w-full lg:w-[90%] md:w-[96%] w-[90%] md:grid-cols-4 items-center gap-8 md:gap-4 lg:gap-8">
        <Slider {...settings}>
          {MembershipPlans.cards.map((card, index) => (
            <Card
              key={index}
              className={`w-full flex flex-col items-center text-center gap-4 border border-zinc-500  transition-all duration-200 cursor-pointer hover:border-red-500/50 ${index === 1 ? "lg:py-16 py-10" : "py-10"}`}
            >
              <img
              ...
              <Button
                className="mt-4 px-6 font-medium text-white py-2.5 bg-gradient-to-r whitespace-nowrap from-red-500 to-amber-500"
                type="button"
